...
{id:'photosynthesis',title:'Quang hợp ở thực vật (Cây rong đuôi chó)',subject:'Sinh học',grade:'Lớp 7',description:'Điều chỉnh cường độ ánh sáng, nồng độ CO₂ và quan sát bọt khí O₂ thoát ra cùng đồ thị quang hợp thời gian thực.',kind:'photosynthesis'},
{id:'earth-sun-orbit',title:'Chuyển động của Trái Đất quanh Mặt Trời & Bốn Mùa',subject:'Địa lý',grade:'Lớp 6',description:'Quan sát Trái Đất nghiêng 23.5° quay quanh Mặt Trời, sự phân bố ánh sáng, hiện tượng ngày - đêm và các mùa trong năm.',kind:'earth-sun-orbit'}];
...
photosynthesis:{title:'Quang hợp ở thực vật (Cây rong đuôi chó)',intro:'Điều chỉnh cường độ ánh sáng và nồng độ CO₂ để quan sát tốc độ sinh bọt khí Oxy.',params:[['Cường độ ánh sáng','%',0,100,50,5],['Nồng độ CO₂','ppm',100,1500,400,50],['Nhiệt độ nước','°C',10,45,25,1]],question:'Khi tắt đèn chiếu sáng (cường độ ánh sáng = 0), hiện tượng gì xảy ra?',answers:['Cây ngừng thoát bọt khí O₂','Cây thoát bọt khí O₂ mạnh hơn','Cây tạo nhiều tinh bột hơn'],correct:0,explanation:'Quang hợp cần ánh sáng để pha sáng diễn ra. Không có ánh sáng, quá trình quang phân ly nước và tạo O₂ dừng lại.',note:'Thí nghiệm mô phỏng hiện tượng quang hợp ở cành rong đuôi chó (Hydrilla). Bọt khí quan sát được chủ yếu là khí Oxy (O₂).'},
'earth-sun-orbit':{title:'Chuyển động của Trái Đất quanh Mặt Trời & Bốn Mùa',intro:'Khám phá quỹ đạo Trái Đất quanh Mặt Trời, góc nghiêng trục 23.5° và sự luân phiên mùa.',params:[['Ngày trong năm','ngày',1,365,172,1],['Vĩ độ quan sát','°B',0,90,21,1],['Tốc độ mô phỏng','x',0,5,1,.5]],question:'Vào ngày Hạ chí (22/6), bán cầu nào nghiêng về phía Mặt Trời nhiều nhất?',answers:['Bán cầu Bắc','Bán cầu Nam','Hai bán cầu như nhau'],correct:0,explanation:'Ngày 22/6, Bán cầu Bắc ngả nhiều nhất về phía Mặt Trời, nhận được nhiều ánh sáng và nhiệt nhất (mùa hè ở Bán cầu Bắc).',note:'Trục Trái Đất nghiêng 23.5° không đổi phương trong không gian. Quỹ đạo chuyển động quanh Mặt Trời là đường elip gần tròn.'}};
...
else if(kind==='photosynthesis'){const [light,co2,temp]=vals,tempFactor=Math.max(0,1-Math.pow((temp-28)/18,2)),rate=((light/(light+25))*(co2/(co2+200))*60*tempFactor);ctx.fillStyle='#e0f2fe';ctx.fillRect(250,80,300,320);ctx.strokeStyle='#0284c7';ctx.lineWidth=4;ctx.strokeRect(250,80,300,320);line(250,110,550,110,'#38bdf8',3);line(400,390,400,160,'#15803d',6);for(let y=180;y<=360;y+=30){line(400,y,340,y-15,'#16a34a',4);line(400,y,460,y-15,'#16a34a',4);line(400,y+15,350,y+5,'#22c55e',3);line(400,y+15,450,y+5,'#22c55e',3)}const lampAlpha=Math.min(1,light/100);ctx.fillStyle='rgba(250,204,21,'+(0.15+0.65*lampAlpha)+')';ctx.beginPath();ctx.moveTo(100,60);ctx.lineTo(250,110);ctx.lineTo(250,390);ctx.lineTo(80,120);ctx.closePath();ctx.fill();dot(90,80,24,'#f59e0b');text('ĐÈN LED',55,40,'#b45309',16);const bubbleCount=Math.floor(rate*0.4);for(let b=0;b<bubbleCount;b++){const by=360-((t*80+b*28)%250),bx=400+Math.sin(b*3+t*4)*22;dot(bx,by,3+(b%3),'rgba(255,255,255,0.85)')}text('Cường độ sáng: '+light+'%',570,130,'#0f766e',18);text('Nồng độ CO₂: '+co2+' ppm',570,170,'#0f766e',18);text('Nhiệt độ nước: '+temp+' °C',570,210,'#0f766e',18);text('Tốc độ bọt O₂: '+rate.toFixed(1)+' bọt/phút',570,260,'#0369a1',20);const statusStr=rate>25?'Quang hợp rất mạnh':rate>5?'Quang hợp ổn định':'Quang hợp yếu/ngừng';text('Trạng thái: '+statusStr,570,300,rate>5?'#15803d':'#b91c1c',17);$('result').textContent='Tốc độ quang hợp: '+rate.toFixed(1)+' bọt O₂/phút · '+statusStr+' · Tinh bột tích lũy: '+(rate*1.5).toFixed(0)+' mg/giờ';}
else if(kind==='earth-sun-orbit'){const [day,lat]=vals,decl=23.44*Math.sin(2*Math.PI*(day-80)/365),seasonNorth=decl>10?'Mùa Hè':decl>-10?(day<180?'Mùa Xuân':'Mùa Thu'):'Mùa Đông',seasonSouth=decl<-10?'Mùa Hè':decl<10?(day<180?'Mùa Thu':'Mùa Xuân'):'Mùa Đông';ctx.fillStyle='#0b1329';ctx.fillRect(0,0,800,450);ctx.strokeStyle='rgba(255,255,255,0.25)';ctx.lineWidth=2;if(ctx.setLineDash)ctx.setLineDash([5,5]);ctx.beginPath();if(ctx.ellipse)ctx.ellipse(400,225,280,140,0,0,Math.PI*2);else ctx.arc(400,225,200,0,Math.PI*2);ctx.stroke();if(ctx.setLineDash)ctx.setLineDash([]);const sunGrad=ctx.createRadialGradient?ctx.createRadialGradient(400,225,10,400,225,50):null;if(sunGrad&&sunGrad.addColorStop){sunGrad.addColorStop(0,'#fef08a');sunGrad.addColorStop(0.5,'#f59e0b');sunGrad.addColorStop(1,'rgba(239,68,68,0)');ctx.fillStyle=sunGrad}else{ctx.fillStyle='#fbbf24'}ctx.beginPath();ctx.arc(400,225,50,0,Math.PI*2);ctx.fill();dot(400,225,22,'#fbbf24');text('MẶT TRỜI',365,232,'#78350f',13);const orbitAngle=(day/365)*2*Math.PI,ex=400+280*Math.cos(orbitAngle),ey=225+140*Math.sin(orbitAngle);dot(ex,ey,18,'#0284c7');const axLen=28,tilt=-23.5*Math.PI/180;line(ex-axLen*Math.sin(tilt),ey-axLen*Math.cos(tilt),ex+axLen*Math.sin(tilt),ey+axLen*Math.cos(tilt),'#f43f5e',3);ctx.fillStyle='rgba(255,255,255,0.3)';ctx.beginPath();ctx.arc(ex,ey,18,orbitAngle+Math.PI/2,orbitAngle-Math.PI/2);ctx.fill();text('21/3 Xuân phân',640,225,'#94a3b8',13);text('22/6 Hạ chí',365,75,'#f59e0b',13);text('23/9 Thu phân',40,225,'#94a3b8',13);text('22/12 Đông chí',355,395,'#38bdf8',13);const latRad=lat*Math.PI/180,declRad=decl*Math.PI/180,cosH=-Math.tan(latRad)*Math.tan(declRad);let daylightHours=12;if(cosH<-1)daylightHours=24;else if(cosH>1)daylightHours=0;else daylightHours=(24/Math.PI)*Math.acos(cosH);text('Ngày '+day+'/365',30,40,'#f8fafc',18);text('Góc nghiêng Mặt Trời δ: '+decl.toFixed(1)+'°',30,70,'#38bdf8',16);text('Bán cầu Bắc ('+lat+'°B): '+seasonNorth,30,100,'#4ade80',16);text('Số giờ chiếu sáng ngày: '+daylightHours.toFixed(1)+' giờ',30,130,'#fde047',16);text('Bán cầu Nam: '+seasonSouth,30,160,'#f472b6',16);$('result').textContent='Ngày '+day+' · Góc nghiêng δ = '+decl.toFixed(1)+'° · Bắc: '+seasonNorth+' ('+daylightHours.toFixed(1)+'h nắng) · Nam: '+seasonSouth;}
}
function tick(now){if(running&&!document.hidden){t+=Math.min((now-last)/1000,.05);if(['ohm','waves','photosynthesis','earth-sun-orbit'].includes(kind))draw()}last=now;raf=requestAnimationFrame(tick)}draw();raf=requestAnimationFrame(tick);window.addEventListener('pagehide',()=>cancelAnimationFrame(raf));
</script></body></html>`;
}
